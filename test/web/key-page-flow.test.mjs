...
test("creates the key without requesting a prompt while the release is unavailable", async () => {
  const events = [];
  const request = async (path) => {
    events.push(path);
    return { id: "9", key: "wb_live_public_secret", status: "active" };
  };
  const result = await flow.createKeyWithPrompt({ request, name: "WorkBuddy Windows", onKeyCreated: () => {}, issuePrompt: false });
  assert.deepEqual(events, ["/api/api-keys"]);
  assert.equal(result.key.id, "9");
  assert.equal(result.prompt, null);
  assert.equal(result.promptSkipped, true);
});

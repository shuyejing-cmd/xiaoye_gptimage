...
function encryptionKey(value) {
  ...
  if (buffer.length !== 32) throw new AppError({ code: "invalid_config", message: "PAYLOAD_ENCRYPTION_KEY must encode exactly 32 bytes", httpStatus: 500 });
  return buffer;
}

export function loadPlatformConfig(env = process.env) {
  requireValues(env, ["DATABASE_URL", "AUTH_PEPPER", "API_KEY_PEPPER", "PAYLOAD_ENCRYPTION_KEY", "COS_SECRET_ID", "COS_SECRET_KEY", "COS_BUCKET", "COS_REGION", "SMTP_HOST", "SMTP_FROM"]);
  ...
  return {
    ...
    payloadEncryptionKey: encryptionKey(env.PAYLOAD_ENCRYPTION_KEY),
    imageProvider,

...
function publicKey(row) {
  return { id: row.id, name: row.name, prefix: row.key_prefix, status: row.status, lastUsedAt: row.last_used_at, createdAt: row.created_at };
}

export function createApiKeyService({ pool, pepper, randomBytes = cryptoRandomBytes, now = () => new Date() }) {
  ...
  return {
    create({ userId, name }) {
      return withTransaction(pool, async (client) => {
        ...
        const row = (await client.query(
          "insert into api_keys(user_id,name,key_prefix,key_hash,status,created_at) values($1,$2,$3,$4,'active',$5) returning *",
          [userId, String(name || "WorkBuddy").trim().slice(0, 80) || "WorkBuddy", prefix, hashKey(pepper, key), now()]
        )).rows[0];
    ...
    async list(userId) {
      ...
      return rows.rows.map(publicKey);
    },
    ...
    async revoke({ userId, keyId }) {
      ...
      return publicKey(result.rows[0]);
    },
    ...
    async importLegacy({ userId, rawKey, name = "Legacy WorkBuddy" }) {
      ...
      if (existing.rowCount) return publicKey(existing.rows[0]);
      const row = (await pool.query("insert into api_keys(user_id,name,key_prefix,key_hash,status,created_at) values($1,$2,$3,$4,'active',$5) returning *", [userId, name, prefix, hashKey(pepper, rawKey), now()])).rows[0];

...
try {
  ...
  & (Join-Path $InstallDir 'runtime\node.exe') (Join-Path $InstallDir 'installer\config-cli.mjs') repair '--gateway=https://xiaoyeai.cn' "--key-file=$KeyFile" "--roots=$($Dialog.SelectedPath)" "--install-dir=$InstallDir"

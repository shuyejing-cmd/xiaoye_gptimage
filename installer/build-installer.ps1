...
New-Item -ItemType Directory -Force (Join-Path $StageDir 'runtime'), (Join-Path $StageDir 'app'), $OutputDir | Out-Null
...
$BootstrapPath = Join-Path $OutputDir 'workbuddy-image-mcp.ps1'
Copy-Item -LiteralPath (Join-Path $PSScriptRoot 'workbuddy-image-mcp-bootstrap.ps1') -Destination $BootstrapPath -Force
$Signed = $false
if ($env:CODE_SIGN_CERT_SHA1) {
  ...
  & $SignTool sign /sha1 $env:CODE_SIGN_CERT_SHA1 /fd SHA256 /tr 'https://timestamp.digicert.com' /td SHA256 $UnversionedInstallerPath
  if ($LASTEXITCODE -ne 0) { throw "Installer signing failed with exit code $LASTEXITCODE." }
  & $SignTool verify /pa $UnversionedInstallerPath
  if ($LASTEXITCODE -ne 0) { throw "Installer signature verification failed with exit code $LASTEXITCODE." }
  $Signature = Get-AuthenticodeSignature -LiteralPath $UnversionedInstallerPath
  ...
  $BootstrapSignature = Set-AuthenticodeSignature -LiteralPath $BootstrapPath -Certificate $Certificate -HashAlgorithm SHA256 -TimestampServer 'https://timestamp.digicert.com'
...
$VersionedInstallerPath = Join-Path $OutputDir $VersionedInstallerName

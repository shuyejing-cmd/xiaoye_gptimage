...
function Keys() {
  ...
  const renderKeyDetail = key => {
    ...
    return <>
      ...
      <section className="key-secret-section"><div className="key-section-title"><h3>个人 Key</h3><span aria-live="polite">{copyState.id === key.id && copyState.type === "key" ? copyState.message : ""}</span></div><div className="key-secret-row"><code className="full-key">{key.key || `wb_live_${key.prefix}_••••••••`}</code>{key.key && <button aria-label="复制完整个人 Key" onClick={() => copy(key.id, "key", key.key)}>复制 Key</button>}</div>{!key.key && key.status === "active" && <p className="key-unrecoverable">完整内容不会再次返回。需要新 Key 时可删除后重新创建。</p>}</section>
